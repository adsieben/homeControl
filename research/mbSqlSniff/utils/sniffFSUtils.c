...
struct option long_options[] = {
    ...
    { "ascfile",     required_argument, NULL, 'a' },
...
void usage(FILE *fp, char *progname, int exit_code)
{
    ...
    fprintf(fp, "%s version %s build %s\n", progname, _VERSION, _BUILD);
    ...
    fprintf(fp, " -a, --ascfile      name of asc file\n");
    fprintf(fp, " -d, --dbfile       name of sqlite database file name\n");
...
void parse_args(int argc, char **argv, struct cli_args *args)
{
    ...
    args->ascFileName = FN_ACT_VALUES;
    args->dbFileName = DB;

    // while ((opt = getopt_long(argc, argv, "ho:p:s:P:S:b:l", long_options, NULL)) >= 0) {
    while ((opt = getopt_long(argc, argv, "vho:p:s:P:S:b:t:a:d:", long_options, NULL)) >= 0) {
        switch (opt) {
        case 'o':
        ...
        case 't':
            ...
            break;
        case 'a':
            args->ascFileName = optarg;
            break;
        case 'd':
            args->dbFileName = optarg;
            break;
        case 'v':
        case 'h':
...
int printTSMeasurement( char * filename, char * fs, time_t datetime, float * msmnt, int size )
{
    FILE *fptr; 
    fptr = fopen( filename, fs); 
    // fptr = fopen( FN_SEC_AVG_VALUES, "a"); 
    fprintf(fptr, "%d ", datetime );
